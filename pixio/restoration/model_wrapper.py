...
class PixIORestoration(nn.Module):
    ...
    def _apply_strategy(self, strategy, lora_opt):
        ...
        if strategy == 'full':
            ...
            target_modules = lora_opt.get('target_modules', ["qkv", "proj", "fc1", "fc2"])
            r = lora_opt.get('r', 16)
            ...
            for mod in decoder_modules:
                if isinstance(mod, torch.Tensor):
                    ...
                    for param in mod.parameters():
                        param.requires_grad = True # Force unfreeze for full finetuning of decoder
    ...
    def forward(self, x):
        ...
        latent, mask, ids_restore = self.encoder.forward_encoder(x, mask_ratio=mask_ratio)
